...
int main() {
  ...
  ll ans{LLONG_MIN};
  auto dfs = [&](auto&& self, ll x, ll y, ll dx, ll dy, ll i, ll p) -> ll {
    if (i == 4) {
      return p;
    }
    ...
    return self(self, x + dx, y + dy, dx, dy, i + 1, p * arr[x][y]);
  };
  for (ll i = 0; i < 20; ++i) {
    for (ll j = 0; j < 20; ++j) {
      ans = max(ans, dfs(dfs, i, j, -1, 1, 0, 1));
      ans = max(ans, dfs(dfs, i, j, 0, 1, 0, 1));
      ans = max(ans, dfs(dfs, i, j, 1, 0, 0, 1));
      ans = max(ans, dfs(dfs, i, j, 1, 1, 0, 1));
    }
  }
  cout << ans << '\n';
